...
- **Synthesize by topic or finding, not by session — and assimilate.** A companion presents the *current best-known facts* for each topic, organized so a reader can follow the state of a question — not a chronological record of when things were found. **When a new finding arrives, assimilate it into the existing topical treatment: rewrite and merge the affected prose so the topic reads as one current account, rather than appending a dated "as of <date>" / "update <date>" block.** When the new finding *corrects or supersedes* an earlier one, revise the earlier statement to the current best understanding — do not leave the superseded claim standing beside the correction. The discovery date and the evolution of a finding are provenance, not body structure — keep them in footnotes or HTML comments, where they remain available as a trail. (A short later paragraph that genuinely *adds* a distinct sub-finding to a topic is fine; a stream of dated session blocks on the same topic is not — merge them.) Open each topic with enough founding context that a reader who has not followed the work understands what it settles. **Citations and footnotes are preserved through every rewrite or merge — assimilation must never drop or weaken a source.**
