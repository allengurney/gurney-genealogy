...
- Highly technical, genealogical, or historical terms which are not in common modern usage should either be a hyperlink to a definition or explanatory source (e.g. wikipedia.org), explained in the prose, a parenthetical synonym, or otherwise avoided for plain English consumption. Examples: "feoffee," "advowsons," "engrailed," "impaling," "in right of his wife," "moiety," "first attested." Give period date-anchors as plain years or spans ("owned since 1086," not "since Domesday Book"; "early January," not "34 Edw. III").
...
- **One footnote per source.** Prefer separate per-source footnotes over one note listing several. When a cluster of related facts each has its own source, narrate the facts in the body — bring them to life — rather than parking a bare list of citations in one long footnote.
- **Separate stacked footnotes with a superscript comma**, not a baseline one: `…</sup><sup>, </sup><sup class="fn">…`, so the comma sits up with the numbers.
