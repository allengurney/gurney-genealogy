...
**Affiliate-library-restricted films — the film opens in the viewer but every leaf is padlocked.** Confirmed 2026-07 for the **NRO Archdeaconry of Norfolk probate-register films** (catalog `koha:291384`, "Wills and administrations, 1459–1857" — e.g. film 008045772, ANF register v.35 Offwood): the authenticated viewer shows only 200-px thumbnails with a padlock ("Image Restricted … Access the site at a FamilySearch Center [or] affiliate library"), and `das/v2/<ark>/dist.jpg` returns **HTTP 403** (not the usual 503→S3 redirect) — a custodian restriction, not a technique gap; do not re-attempt the das pull. The same county's **manor-court films (e.g. DGS 004389483, 004389244) are unrestricted and das-pullable** — the lock is per-custodian-contract, so test one leaf (403 vs S3 redirect) before staging a batch. For pre-1800 Norfolk archdeaconry wills specifically, FindMyPast and Ancestry carry index-only rows (linking out to the NRO copy-order service) and the NRO's own digitised will images start at 1800 — so the read routes are a FamilySearch affiliate library, the NRO in person, or a paid NRO copy.
