...
npm run build        # builds _site/ from all source files
npm run watch        # rebuilds automatically as you edit
```

Deploy: drag the contents of `_site/` into Cloudflare Pages (same process as before).
...
1. `_data/ancestors.json` is the authoritative ancestor dataset — 37+ ancestors with full genealogical data, geographic coordinates (from a prior CSV export), spouses, children, buttons, and land holdings.
2. The site uses Eleventy. The two data-driven pages (`index.njk`, `ancestor-table.njk`) loop over `ancestors.json` using Nunjucks `{% for item in ancestors %}`. No JavaScript is involved in rendering.
...
6. To deploy: run `npm run build`, then deploy the `_site/` directory to Cloudflare Pages.
