...
> **Revised 2026-07-12 (Allen's direction): extra-light hub.** The original
> heavy-hub design below is superseded. With the topic library, the case file,
> and the website research-library landing page all carrying the substance, a
> 2,500–4,000 word synthesis at the root is a redundant, lower-value artifact
> whose only guaranteed effect is a second copy to keep in sync. Human readers
> use the website; AI grounds via the repo search tool over the topic files.
> The root companion is therefore an **extra-light entry-point stub**, not a
> synthesis. The staged hub is `hub.md` (manifest `website.rootHubFile`); at
> cutover it replaces `g13-john-gurney-fact-sheet.research.md`.
>
> The stub should carry:
> - a condensed identity/lineage banner (a few footnoted anchor facts);
> - an orientation section pointing to the research library, fact sheet, case
>   file, and the direct-line father/son companions;
> - an identity-status statement that references the case file's probability
>   model as external-canonical (the ~65% figure lives in the case file, not
>   restated as a project fact here).
>
> It must **not** duplicate the topic content: no reproduced known-facts table,
> per-domain synthesis, conflicts list, open-questions list, topic index, or
> crosslink map — those are owned by the topic units and generated by the
> website. Target band: roughly **300–800 words**. "Satisfy the companion rule"
> is met by a genuine, cited entry point, not by length.
