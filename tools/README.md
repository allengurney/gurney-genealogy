...
- **`g13_graph.py`** — standalone lifecycle CLI for the canonical G13 SQLite
  context graph. Phase G0/G1A provides schema migration, source-registry sync,
  synthetic seeding, validation, deterministic recovery export/restore, status,
  and a basic item read without changing `repo_search.py`. See
  `g13_graph/README.md`.
